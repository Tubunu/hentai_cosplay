...
import 'nucosplay_detail_page.dart';
import 'widgets/nucosplay_card.dart';

class NucosplayBrowsePage extends StatefulWidget {
  const NucosplayBrowsePage({super.key});

  @override
  State<NucosplayBrowsePage> createState() => _NucosplayBrowsePageState();
}

class _NucosplayBrowsePageState extends State<NucosplayBrowsePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final prov = context.read<NucosplayBrowseProvider>();
  ...
  void _showRangeDownloadDialog(BuildContext context, NucosplayBrowseProvider provider) {
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    for (int p = startPage; p <= endPage; p++) {
                      try {
                        final res = await NucosplayApiService.fetchPageData(
                          page: p,
                          category: category,
                          keyword: kw,
                        );
                        ...
                        debugPrint('[NuCosplay Range Download] Error on page $p: $e');
                      }
                    }
  ...
  void _showJumpPageDialog(BuildContext context, NucosplayBrowseProvider provider) {
    JumpPageDialog.show(
      context,
      currentPage: provider.currentPage,
      totalPages: provider.totalPages,
      themeColor: const Color(0xFF7E57C2),
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<NucosplayBrowseProvider>();
    final isDark = Theme.of(context).brightness == Brightness.dark;
    const themeColor = Color(0xFF7E57C2);
                      ...
                      child: Row(
                        children: [
                          ...
                          RandomActionButton.album(
                            albumSource: MediaSourceType.nucosplay,
                            isCapsule: true,
                            color: themeColor,
                          ),
                          const SizedBox(width: 8),
                  ...
                  SliverToBoxAdapter(
                    child: SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: NuCosplayCategory.values.map((cat) {
                      ...
                      sliver: SliverGrid(
                        ...
                        delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            ...
                            return NucosplayCard(
                              item: item,
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => NucosplayDetailPage(item: item),
                                  ),
                                );
                              },
                            );
