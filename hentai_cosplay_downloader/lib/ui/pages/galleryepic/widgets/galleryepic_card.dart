...
class GalleryepicCard extends StatelessWidget {
  final AlbumItem item;
  final VoidCallback onTap;

  const GalleryepicCard({
    super.key,
    required this.item,
    required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final isSelected = context.select<GalleryepicBrowseProvider, bool>((p) => p.isSelected(item));
    final isSelectionMode = context.select<GalleryepicBrowseProvider, bool>((p) => p.isSelectionMode);
    ...
    final tagBadge = item.tags.isNotEmpty ? item.tags.first : 'GalleryEpic';

    return BouncingButton(
      onTap: () {
        if (isSelectionMode) {
          context.read<GalleryepicBrowseProvider>().toggleItemSelection(item);
        } else {
          onTap();
        }
      },
                ...
                children: [
                  ...
                  if (item.date.isNotEmpty)
                    Positioned(
                      ...
                      child: Row(
                        children: [
                          const Icon(CupertinoIcons.photo_fill_on_rectangle_fill, size: 10, color: Colors.white70),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          item.author.isNotEmpty ? item.author : 'GalleryEpic',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 11,
