...
class IwaraVideoCard extends StatelessWidget {
  final VideoItem item;
  final VoidCallback onTap;

  const IwaraVideoCard({
    super.key,
    required this.item,
    required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final isSelected = context.select<IwaraBrowseProvider, bool>((p) => p.isSelected(item));
    final isSelectionMode = context.select<IwaraBrowseProvider, bool>((p) => p.isSelectionMode);
    ...
    final broadcaster = item.author.isNotEmpty ? item.author : 'Iwara';
    final numViews = item.rawData['numViews'] as int? ?? 0;
    final numLikes = item.rawData['numLikes'] as int? ?? 0;
    final durationFormatted = item.rawData['durationFormatted'] as String? ?? '';
    final rating = item.rawData['rating'] as String? ?? 'general';

    return BouncingButton(
      onTap: () {
        if (isSelectionMode) {
          context.read<IwaraBrowseProvider>().toggleItemSelection(item);
        } else {
          onTap();
        }
      },
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: (rating == 'ecchi' ? const Color(0xFFFF2E63) : const Color(0xFF00A8FF))
                  ...
                  if (numViews > 0 || numLikes > 0)
                    Positioned(
                      ...
                      child: Row(
                        children: [
                          if (numLikes > 0) ...[
                            const Icon(CupertinoIcons.heart_fill, size: 10, color: Color(0xFFFF4081)),
                            const SizedBox(width: 3),
                            Text(
                              numLikes >= 1000 ? '${(numLikes / 1000).toStringAsFixed(1)}k' : '$numLikes',
                              style: const TextStyle(color: Colors.white, fontSize: 10.5, fontWeight: FontWeight.bold),
                            ),
                            const SizedBox(width: 6),
                          ],
                          if (numViews > 0) ...[
                            const Icon(CupertinoIcons.play_fill, size: 10, color: Colors.white70),
                            const SizedBox(width: 2),
                            Text(
                              numViews >= 10000
                                  ? '${(numViews / 10000).toStringAsFixed(1)}w'
                                  : (numViews >= 1000 ? '${(numViews / 1000).toStringAsFixed(1)}k' : '$numViews'),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      const Icon(CupertinoIcons.person_alt_circle_fill, size: 13, color: themeColor),
                      const SizedBox(width: 4),
                      Expanded(
                        child: Text(
                          broadcaster,
