...
class NjavDetailPage extends StatefulWidget {
  final VideoItem item;

  const NjavDetailPage({super.key, required this.item});

  @override
  State<NjavDetailPage> createState() => _NjavDetailPageState();
}

class _NjavDetailPageState extends State<NjavDetailPage> {
  final ScrollController _scrollController = ScrollController();
  late VideoItem _item;
  bool _isLoading = true;
  String? _errorMessage;

  static const _themeColor = Color(0xFFFE628E);
  ...
  Future<void> _resolveDetail() async {
    ...
    try {
      final resolved = await NjavApiService.resolveVideoDetail(_item);
      if (mounted) {
        setState(() {
          _item = resolved;
          _isLoading = false;
  ...
  void _openWebPlayer() {
    final embedUrl = _item.rawData['embed_url']?.toString() ?? 'https://www.njav.com/vv/${_item.slug}';
  ...
  void _tryNativePlayer() {
    final preview = _item.rawData['preview_video']?.toString() ?? '';
    final videoUrl = _item.videoUrl ??
        (preview.isNotEmpty
            ? preview
            : (_item.rawData['embed_url']?.toString() ?? 'https://www.njav.com/vv/${_item.slug}'));
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  '部分信息加载异常，已启用内置解析: $_errorMessage',
                          ...
                          CupertinoButton(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text(
                                  previewVideo.isNotEmpty ? '预告/内置' : '内置尝试',
                                  style: TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.w600,
                                    color: isDark ? Colors.white70 : Colors.black87,
