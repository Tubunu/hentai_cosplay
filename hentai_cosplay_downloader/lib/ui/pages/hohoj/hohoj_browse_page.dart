...
import 'hohoj_detail_page.dart';
import 'widgets/hohoj_video_card.dart';

class HohojBrowsePage extends StatefulWidget {
  const HohojBrowsePage({super.key});

  @override
  State<HohojBrowsePage> createState() => _HohojBrowsePageState();
}

class _HohojBrowsePageState extends State<HohojBrowsePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final provider = context.read<HohojBrowseProvider>();
  ...
  Widget build(BuildContext context) {
    ...
    final provider = context.watch<HohojBrowseProvider>();
                    ...
                    SliverToBoxAdapter(
                      child: Container(
                        ...
                        child: ListView.separated(
                          ...
                          itemCount: HohojCategory.values.length,
                          separatorBuilder: (_, __) => const SizedBox(width: 8),
                          itemBuilder: (context, idx) {
                            final cat = HohojCategory.values[idx];
                    ...
                    SliverToBoxAdapter(
                      child: Container(
                        ...
                        child: ListView.separated(
                          scrollDirection: Axis.horizontal,
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          itemCount: HohojOrder.values.length,
                          separatorBuilder: (_, __) => const SizedBox(width: 6),
                          itemBuilder: (context, idx) {
                            final ord = HohojOrder.values[idx];
                            final isCurrent = provider.currentOrder == ord;
                            return Center(
                              child: BouncingButton(
                                onTap: () {
                                  if (!isCurrent) {
                                    provider.setOrder(ord);
                                    _scrollToTop();
                                  }
                                },
                                child: AnimatedContainer(
                                  ...
                                  decoration: BoxDecoration(
                                    color: isCurrent
                                        ? (isDark ? const Color(0xFF3A2020) : const Color(0xFFFFECEB))
                                        : Colors.transparent,
                      ...
                      sliver: SliverGrid(
                        ...
                        delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            ...
                            return HohojVideoCard(
                              item: item,
                              onTap: () {
                                Navigator.of(context).push(
                                  CupertinoPageRoute(
                                    builder: (_) => HohojDetailPage(item: item),
                                  ),
                                );
                              },
                            );
