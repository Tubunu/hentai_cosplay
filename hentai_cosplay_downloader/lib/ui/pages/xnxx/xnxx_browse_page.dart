...
import 'widgets/xnxx_video_card.dart';
import 'xnxx_detail_page.dart';

class XnxxBrowsePage extends StatefulWidget {
  const XnxxBrowsePage({super.key});

  @override
  State<XnxxBrowsePage> createState() => _XnxxBrowsePageState();
}

class _XnxxBrowsePageState extends State<XnxxBrowsePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final provider = context.read<XnxxBrowseProvider>();
  ...
  Widget build(BuildContext context) {
    ...
    final provider = context.watch<XnxxBrowseProvider>();
                        ...
                        Container(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText: '搜索 XNXX (如 Cosplay, Asian, Japanese)...',
                              hintStyle: TextStyle(
                                fontSize: 12,
                        ...
                        SingleChildScrollView(
                          ...
                          child: Row(
                            children: [
                              ...
                              // Best month switcher chip (if category is Best)
                              if (provider.category == XnxxCategory.best && !provider.isSearchMode) ...[
                                Padding(
                                  padding: const EdgeInsets.only(right: 8),
                                  child: PopupMenuButton<String>(
                                    initialValue: provider.bestMonth,
                                    onSelected: (m) {
                                      provider.selectBestMonth(m);
                                      _scrollToTop();
                                    },
                                    itemBuilder: (_) => XnxxApiService.getAvailableBestMonths().map((m) {
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(
                                        children: [
                                          const Icon(CupertinoIcons.calendar, size: 14, color: themeColor),
                                          ...
                                          const Icon(CupertinoIcons.chevron_down, size: 12, color: themeColor),
                                        ],
                                      ),
                                    ),
                                  ),
                    ...
                    sliver: SliverGrid(
                      ...
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          ...
                          return XnxxVideoCard(
                            item: item,
                            onTap: () {
                              Navigator.push(
                                context,
                                CupertinoPageRoute(
                                  builder: (_) => XnxxDetailPage(item: item),
                                ),
                              );
                            },
                          );
