...
class JavguruDetailPage extends StatefulWidget {
  final VideoItem item;

  const JavguruDetailPage({super.key, required this.item});

  @override
  State<JavguruDetailPage> createState() => _JavguruDetailPageState();
}

class _JavguruDetailPageState extends State<JavguruDetailPage> {
  final ScrollController _scrollController = ScrollController();
  late VideoItem _item;
  bool _isLoading = true;
  String? _errorMessage;
  JavguruDetailData? _detailData;
  ...
  Future<void> _resolveDetail() async {
    ...
    try {
      final detail = await JavguruApiService.fetchDetail(_item);
      if (mounted) {
        setState(() {
          _detailData = detail;
          _item = detail.item;
  ...
  Future<void> _playVideo([String? specificEmbedUrl, bool forceWeb = false]) async {
    ...
    if (embedUrl.endsWith('.mp4') || embedUrl.endsWith('.m3u8')) {
      VideoPlayerPage.openRemote(
        ...
        headers: const {'Referer': 'https://jav.guru/'},
      );
      return;
    }
    ...
    try {
      final streamUrl = await JavguruApiService.resolveStreamUrl(embedUrl);
      if (!mounted) return;
      setState(() {
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          if (_detailData != null && _detailData!.studio != null)
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 4),
                              decoration: BoxDecoration(
                                color: isDark ? const Color(0xFF2C2C2E) : const Color(0xFFE5E5EA),
                      ...
                      // Server Line Switcher (if multiple streams available)
