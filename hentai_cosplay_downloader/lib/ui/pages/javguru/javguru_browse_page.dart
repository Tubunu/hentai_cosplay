...
import 'javguru_detail_page.dart';
import 'widgets/javguru_video_card.dart';

class JavguruBrowsePage extends StatefulWidget {
  const JavguruBrowsePage({super.key});

  @override
  State<JavguruBrowsePage> createState() => _JavguruBrowsePageState();
}

class _JavguruBrowsePageState extends State<JavguruBrowsePage> {
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _searchController = TextEditingController();
  static const _themeColor = Color(0xFF00ADB5);

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final provider = context.read<JavguruBrowseProvider>();
  ...
  void _onSearch(String keyword) {
    context.read<JavguruBrowseProvider>().search(keyword);
  }

  void _onCategorySelected(JavguruCategory category) {
    _searchController.clear();
    context.read<JavguruBrowseProvider>().setCategory(category);
  }
  ...
  void _batchDownload(List<VideoItem> selectedItems) {
    ...
    context.read<JavguruBrowseProvider>().clearSelection();
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<JavguruBrowseProvider>();
                    ...
                    children: [
                      Expanded(
                        child: CupertinoSearchTextField(
                          ...
                          placeholder: '搜索 JavGuru 影片番号、演员或关键词...',
                      ...
                      RandomActionButton.video(
                        videoSite: VideoSiteType.javguru,
                        isCapsule: true,
                        color: _themeColor,
                      ),
                      const SizedBox(width: 8),
              ...
              SliverToBoxAdapter(
                child: SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemCount: JavguruCategory.values.length,
                    separatorBuilder: (_, __) => const SizedBox(width: 8),
                    itemBuilder: (context, index) {
                      final cat = JavguruCategory.values[index];
                      final isSelected =
                  ...
                  sliver: SliverGrid(
                    ...
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        return JavguruVideoCard(
                          item: item,
                          onTap: () {
                            Navigator.of(context).push(
                              CupertinoPageRoute(
                                builder: (_) => JavguruDetailPage(item: item),
                              ),
                            );
                          },
                        );
