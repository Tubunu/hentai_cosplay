...
class JavmostDetailPage extends StatefulWidget {
  final VideoItem item;

  const JavmostDetailPage({super.key, required this.item});

  @override
  State<JavmostDetailPage> createState() => _JavmostDetailPageState();
}

class _JavmostDetailPageState extends State<JavmostDetailPage> {
  final ScrollController _scrollController = ScrollController();
  late VideoItem _item;
  bool _isLoading = true;
  String? _errorMessage;
  JavmostDetailData? _detailData;
  ...
  Future<void> _resolveDetail() async {
    ...
    try {
      final detail = await JavmostApiService.fetchDetail(_item);
      if (mounted) {
        setState(() {
          _detailData = detail;
          _item = detail.item;
  ...
  Future<void> _playVideo([String? specificEmbedUrl, bool forceWeb = false]) async {
    ...
    if (embedUrl.endsWith('.mp4') || embedUrl.endsWith('.m3u8')) {
      VideoPlayerPage.openRemote(
        ...
        headers: const {'Referer': 'https://www.javmost.ws/'},
      );
      return;
    }
    ...
    try {
      final streamUrl = await JavmostApiService.resolveStreamUrl(_item.detailUrl);
      if (!mounted) return;
      setState(() {
      ...
      if (streamUrl != null && streamUrl.isNotEmpty) {
        VideoPlayerPage.openRemote(
          ...
          webPlayerUrl: _item.detailUrl,
          headers: const {
            'Referer': 'https://www.dooplayer.com/',
