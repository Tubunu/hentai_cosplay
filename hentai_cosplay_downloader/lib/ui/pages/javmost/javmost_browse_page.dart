...
import 'javmost_detail_page.dart';
import 'widgets/javmost_video_card.dart';

class JavmostBrowsePage extends StatefulWidget {
  const JavmostBrowsePage({super.key});

  @override
  State<JavmostBrowsePage> createState() => _JavmostBrowsePageState();
}

class _JavmostBrowsePageState extends State<JavmostBrowsePage> {
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _searchController = TextEditingController();
  static const _themeColor = Color(0xFFA80000);

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final provider = context.read<JavmostBrowseProvider>();
  ...
  void _onSearch(String keyword) {
    context.read<JavmostBrowseProvider>().search(keyword);
  }

  void _onCategorySelected(JavmostCategory category) {
    _searchController.clear();
    context.read<JavmostBrowseProvider>().setCategory(category);
  }
  ...
  void _batchDownload(List<VideoItem> selectedItems) {
    ...
    context.read<JavmostBrowseProvider>().clearSelection();
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<JavmostBrowseProvider>();
                    ...
                    children: [
                      Expanded(
                        child: CupertinoSearchTextField(
                          ...
                          placeholder: '搜索 JavMost 影片番号、演员或关键词...',
                      ...
                      RandomActionButton.video(
                        videoSite: VideoSiteType.javmost,
                        isCapsule: true,
                        color: _themeColor,
                      ),
                      const SizedBox(width: 8),
              ...
              SliverToBoxAdapter(
                child: SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemCount: JavmostCategory.values.length,
                    separatorBuilder: (_, __) => const SizedBox(width: 8),
                    itemBuilder: (context, index) {
                      final cat = JavmostCategory.values[index];
                      final isSelected =
                  ...
                  sliver: SliverGrid(
                    ...
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        return JavmostVideoCard(
                          item: item,
                          onTap: () {
                            Navigator.of(context).push(
                              CupertinoPageRoute(
                                builder: (_) => JavmostDetailPage(item: item),
                              ),
                            );
                          },
                        );
