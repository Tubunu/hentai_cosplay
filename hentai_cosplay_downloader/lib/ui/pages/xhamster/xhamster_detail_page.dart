...
class XhamsterDetailPage extends StatefulWidget {
  final VideoItem item;

  const XhamsterDetailPage({super.key, required this.item});

  @override
  State<XhamsterDetailPage> createState() => _XhamsterDetailPageState();
}

class _XhamsterDetailPageState extends State<XhamsterDetailPage> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<BrowsingHistoryProvider>().recordVideo(
          ...
          siteKey: 'xhamster',
          siteName: 'xHamster',
          siteColor: const Color(0xFFD32F2F),
        );
      }
    });
  }
  ...
  Future<void> _resolveDetail() async {
    ...
    try {
      final resolved = await XhamsterApiService.resolveVideoDetail(_item);
      if (mounted) {
        setState(() {
          _item = resolved;
          _isLoading = false;
