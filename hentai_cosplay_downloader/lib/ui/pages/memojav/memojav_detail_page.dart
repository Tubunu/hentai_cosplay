...
class MemojavDetailPage extends StatefulWidget {
  final VideoItem item;

  const MemojavDetailPage({super.key, required this.item});

  @override
  State<MemojavDetailPage> createState() => _MemojavDetailPageState();
}

class _MemojavDetailPageState extends State<MemojavDetailPage> {
  ...
  Future<void> _resolveDetail() async {
    ...
    try {
      final resolved = await MemojavApiService.resolveVideoDetail(_item);
      if (mounted) {
        setState(() {
          _item = resolved;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final duration = _item.duration.isNotEmpty
        ? _item.duration
        : (_item.rawData['duration'] as String? ?? '');
    final metadata = _item.rawData['metadata'] as String? ?? '';
    final screenshots = (_item.rawData['screenshots'] as List<dynamic>?)?.cast<String>() ?? [];
                      ...
                      Row(
                        children: [
                          ...
                          if (duration.isNotEmpty) ...[
                            ...
                            Icon(CupertinoIcons.time, size: 14, color: isDark ? Colors.white54 : Colors.black45),
                            ...
                            itemBuilder: (context, idx) {
                              ...
                              return ClipRRect(
                                ...
                                child: CachedNetworkImage(
                                  ...
                                  errorWidget: (_, __, ___) => Container(
                                    color: isDark ? const Color(0xFF2C2C2E) : const Color(0xFFE5E5EA),
                                    child: const Icon(CupertinoIcons.photo, color: Colors.white38),
                                  ),
                                ),
                              );
                            },
