...
class CosvaultCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isSelected = context.select<CosvaultBrowseProvider, bool>((p) => p.isSelected(item));
    final isSelectionMode = context.select<CosvaultBrowseProvider, bool>((p) => p.isSelectionMode);
    ...
    final tagBadge = item.tags.isNotEmpty ? item.tags.first : 'CosVault';

    return BouncingButton(
      onTap: () {
        if (isSelectionMode) {
          context.read<CosvaultBrowseProvider>().toggleItemSelection(item);
        } else {
          onTap();
        }
      },
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          item.author.isNotEmpty ? item.author : 'CosVault',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 11,
