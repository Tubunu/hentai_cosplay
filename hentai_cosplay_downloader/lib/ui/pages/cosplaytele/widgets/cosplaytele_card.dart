...
class CosplayteleCard extends StatelessWidget {
  final AlbumItem item;
  final VoidCallback onTap;

  const CosplayteleCard({
    super.key,
    required this.item,
    required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final isSelected = context.select<CosplayteleBrowseProvider, bool>((p) => p.isSelected(item));
    final isSelectionMode = context.select<CosplayteleBrowseProvider, bool>((p) => p.isSelectionMode);
    ...
    final category = item.tags.isNotEmpty ? item.tags.first : 'CosplayTele';

    return BouncingButton(
      onTap: () {
        if (isSelectionMode) {
          context.read<CosplayteleBrowseProvider>().toggleItemSelection(item);
        } else {
          onTap();
        }
      },
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          item.author.isNotEmpty ? item.author : 'CosplayTele',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 11,
