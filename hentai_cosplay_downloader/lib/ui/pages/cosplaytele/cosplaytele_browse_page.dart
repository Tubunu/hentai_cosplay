...
import 'cosplaytele_detail_page.dart';
import 'widgets/cosplaytele_card.dart';

class CosplayteleBrowsePage extends StatefulWidget {
  const CosplayteleBrowsePage({super.key});

  @override
  State<CosplayteleBrowsePage> createState() => _CosplayteleBrowsePageState();
}

class _CosplayteleBrowsePageState extends State<CosplayteleBrowsePage> {
  ...
  void _showRangeDownloadDialog(BuildContext context, CosplayteleBrowseProvider provider) {
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    for (int p = startPage; p <= endPage; p++) {
                      try {
                        final res = await CosplayteleApiService.fetchPageData(
                          page: p,
                          category: category,
                          keyword: kw,
                        );
                        ...
                        debugPrint('[CosplayTele Range Download] Error on page $p: $e');
                      }
                    }
  ...
  void _showJumpPageDialog(BuildContext context, CosplayteleBrowseProvider provider) {
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<CosplayteleBrowseProvider>();
    final isDark = Theme.of(context).brightness == Brightness.dark;
    const themeColor = Color(0xFF00ACC1);
                  ...
                  SliverToBoxAdapter(
                    child: SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: CosplayTeleCategory.values.map((cat) {
                      ...
                      sliver: SliverGrid(
                        ...
                        delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            ...
                            return CosplayteleCard(
                              item: item,
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => CosplayteleDetailPage(item: item),
                                  ),
                                );
                              },
                            );
