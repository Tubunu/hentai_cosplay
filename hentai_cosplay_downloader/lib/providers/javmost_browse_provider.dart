...
class JavmostBrowseProvider extends ChangeNotifier {
  ...
  JavmostCategory _currentCategory = JavmostCategory.all;
  ...
  JavmostCategory get currentCategory => _currentCategory;
  ...
  JavmostBrowseProvider({bool autoLoad = true}) {
    if (autoLoad) {
      loadVideos();
    }
  }
  ...
  Future<void> loadVideos({int page = 1}) async {
    ...
    try {
      final data = await JavmostApiService.fetchVideos(
        page: page,
  ...
  Future<void> setCategory(JavmostCategory category) async {
