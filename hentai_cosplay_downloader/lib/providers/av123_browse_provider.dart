...
class Av123BrowseProvider extends ChangeNotifier {
  ...
  Av123Category _currentCategory = Av123Category.newest;
  ...
  Av123Category get currentCategory => _currentCategory;
  ...
  Av123BrowseProvider({bool autoLoad = true}) {
    if (autoLoad) {
      loadVideos();
    }
  }
  ...
  Future<void> loadVideos({int page = 1}) async {
    ...
    try {
      final data = await Av123ApiService.fetchVideos(
        page: page,
  ...
  Future<void> setCategory(Av123Category category) async {
